...
public class RentAlManager implements RentAlService{
	...
	public Result add(AddRentAlRequest addRentAlRequest) {
		...
		customers.setFindeksScore(this.customerDao.getFindeksScoreByCustomerId(customers.getCustomerId()));
		
		Car car=new Car();
		car.setCarId(addRentAlRequest.getCarId());
		car.setCarName(addRentAlRequest.getCarName());
		car.setCarFindeksScore(this.carDao.getFindeksScoreByCarId(car.getCarId()));
		
		rentAl.setCar(car);
		rentAl.setCustomers(customers);
		
		var result= BusinnesRules.run(checkCarIsSubmit(rentAl.getCar().getCarId()),checkFindeksScoreCenter(car, customers));
		if (result!=null) {
			return result;
		}
	...
	private Result checkFindeksScoreCenter(Car car, Customers customers) {
		if (customers.getFindeksScore()<car.getCarFindeksScore()) {
